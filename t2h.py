...
def old_test_connection(host_1,host_2):
	...
	if len(output) < 2:
		return [False,"Host1 can not reach Host 2"]
	return [True,output[1]]

def web_test(host_1,host_2):
	if host_1 not in list_of_hosts:
		return [False,"Host 1 not in List of Hosts"]
	if host_2 not in list_of_hosts:
                return [False,"Host 2 not in List of Hosts"]
	return old_test_connection(host_1,host_2)
...
def main():
	...
	print old_test_connection(test_h_name_1,test_h_name_2)
